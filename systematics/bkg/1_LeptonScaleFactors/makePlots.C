...
int makePlots(double ptCut, double ecaloCut, double iasCut){
  ...
  */
  ...
  pred.histoASmiSR   = new TH1D("MC_pred_SRbins","pred_SRbins",2,xbinsASmiSR);
  pred.histoASmi     = new TH1D("MC_pred","pred",4,xbinsASmi);
  pred.histoNValidSR = new TH1D("MC_pred","pred",3,xbinsNHits);
  pred.histoECalo    = new TH1D("predECalo","predECalo",2,xbinsECalo);
  pred.histoPt       = new TH1D("predPt","predPt",5,xbinsPt);
  SR.histoASmiSR     = new TH1D("MC_SR_SRbins","SR_SRbins",2,xbinsASmiSR);
  SR.histoASmi       = new TH1D("MC_SR","SR",4,xbinsASmi);
  SR.histoNValidSR   = new TH1D("MC_SR","SR",3,xbinsNHits); 
  SR.histoECalo      = new TH1D("srECalo","srECalo",2,xbinsECalo);
  SR.histoPt         = new TH1D("SRPt","SRPt",5,xbinsPt);
  data.histoASmiSR   = new TH1D("data_pred_SRbins","data_SRbins",2,xbinsASmiSR);
  data.histoASmi     = new TH1D("data_pred","data",4,xbinsASmi);
  data.histoNValidSR = new TH1D("data_pred","data",3,xbinsNHits); 
  data.histoECalo    = new TH1D("dataECalo","dataECalo",2,xbinsECalo);
  data.histoPt       = new TH1D("dataPt","dataPt",5,xbinsPt);
  ...
  TCanvas *c1 = drawRatioPlotWithPurity(pred.histoASmiSR,SR.histoASmiSR,"I_{as} fakes","plotsOnlyTrackCutsNonInvertedD0Cut/" + filename + pred.histoASmiSR->GetName() + "_"  + SR.histoASmiSR->GetName() + ".pdf" ,pred.purity);
  filename = "hASmi_d0Inverted_fakes_";
  filename += Form("ECalaoLe%.0f_",ecaloCut);
  filename += Form("trackPtGt%.0f_",ptCut);
  TCanvas *c2 = drawRatioPlotWithPurity(pred.histoASmi,SR.histoASmi,"I_{as} fakes","plotsOnlyTrackCutsNonInvertedD0Cut/" + filename + pred.histoASmi->GetName() + "_"  + SR.histoASmi->GetName() + ".pdf" ,pred.purity);
   //TCanvas *c1a = drawRatioPlotWithPurity(pred.histoPt,SR.histoPt,"pt fakes","trackPt_fakes" ,pred.purity,LowECaloRegion);
  //TCanvas *c2 = drawRatioPlotWithPurity(pred.histoASmiSR,data.histoASmiSR,"I_{as} fakes",filename + pred.histoASmiSR->GetName() + "_"  + data.histoASmiSR->GetName() + ".pdf" ,pred.purity,LowECaloRegion);
  ...
  //TCanvas *c2 = drawRatioPlot(pred.histoNValidSR,SR.histoNValidSR,"n_{Hits} fakes","hNValid_SRbinning_d0Inverted_fakes.pdf");
